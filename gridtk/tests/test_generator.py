...
def test_aggregation():
    ...
    template = (
        "{% for k in cfgset %}{{ k.name }} - {{ k.version }}\n{% endfor %}"
    )
...
def test_cmdline_generation():
    ...
    try:
        ...
        with open(variables, "wt") as f:
            f.write(data)
        gentmpl = os.path.join(tmpdir, "gentmpl.txt")
        with open(gentmpl, "wt") as f:
            f.write(template)
...
def test_cmdline_aggregation():
    ...
    try:
        ...
        with open(aggtmpl_file, "wt") as f:
            f.write(aggtmpl)
        aggout = os.path.join(tmpdir, "out", "agg.txt")

        nose.tools.eq_(
            jgen.main(
                ["-vv", variables, gentmpl, genout, aggtmpl_file, aggout]
            ),
            0,
        )
...
"""
    ...
    try:
        ...
        nose.tools.eq_(
            jgen.main(
                ["-vv", "-u", variables, gentmpl, genout, aggtmpl_file, aggout]
            ),
            0,
        )

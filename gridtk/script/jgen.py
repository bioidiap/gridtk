...
def main(command_line_options=None):
    ...
    parser = argparse.ArgumentParser(
        description=__doc__, epilog=epilog, formatter_class=formatter
    )

    parser.add_argument(
        "variables",
        type=str,
        help="Text file containing the variables in YAML format",
    )
    parser.add_argument(
        "gentmpl",
        type=str,
        help="Text file containing the template for generating multiple outputs, one for each configuration set",
    )
    parser.add_argument(
        "genout", type=str, help="Template for generating the output filenames"
    )
    parser.add_argument(
        "aggtmpl",
        type=str,
        nargs="?",
        help="Text file containing the template for generating one single output out of all configuration sets",
    )
    parser.add_argument(
        "aggout",
        type=str,
        nargs="?",
        help="Name of the output aggregation file",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        dest="unique",
        action="store_true",
        help="It will make sure the output lines in aggout are unique while ignoring the empty lines and comment lines.",
    )
